...
(function(){
	if($.Datatype){
		...
		$.extend($.Datatype,{
           ...
           "numrangebt":function(gets,obj,curform,datatype){
				/*
					判断数值范围
					注意需要给表单元素绑定max,min属性来指定最大或最小可输入的值，没有绑定的话使用默认值
				*/
                ...
                if(max=="0"){
                var min_num=parseFloat($("#"+min).val());
                   max_num=parseFloat(gets);
                   //alert(max_num<min_num);
                   if(max_num<min_num){
					return "供货价不能大于实际销售价！";
				   }
                }
                if(min=="0"){
                  var max_num=parseFloat($("#"+max).val());
                   min_num=parseFloat(gets);
                    //alert(max_num<min_num);
                   if(max_num<min_num){
					return "供货价不能大于实际销售价！";
				}
...
//				}
                  var max=obj.attr("max");
					var min=obj.attr("min");
                    var gets=Date.parse(gets.replace(/-/g, "/"));
					 gets=new Date(gets);
