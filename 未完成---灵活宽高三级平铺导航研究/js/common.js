	...
	$(_arr).hover(function(){
		...
		if($(this).find(_down).is(":animated")){
	       $(this).find(_down).stop(true,true);
			...
			$(this).find(_down).delay(50).slideDown(300);
		},function(){
			if($(this).find(_down).is(":animated")){
	           $(this).find(_down).stop(true,true);
		       }
...
function navAlign(){
	...
	li.mouseenter(function(){
		...
		//设置偏移量
		nav.css({"margin-left":-minus,"clear":"both"});
		//循环li找第一个
		li.each(function(i,d){
			if($(d).offset().left-ulOffsetLeft == 0&&i!=0){a.push(i);}
		});
		//console.log(a)
		var _index = $(this).index();
		a.push(_index);
		...
		setTimeout(function(){
			...
			var clone = $("<div class='clone111'></div>")
			clone.css({"width":navWidth,"height":navHeight,"float":"left"});
			li.eq(a[row]-1).after(clone);
		},50)
	});
