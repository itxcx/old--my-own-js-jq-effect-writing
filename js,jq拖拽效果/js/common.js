...
function convey(){
	...
	ul.width(liWidth);
//中间加class
	li.eq(parseInt(li.length/3/2+li.length/3)).addClass("sel");
//ul定位
	var startLeft = -(oneLiWidth*5);
	...
	function lBtnF(){
		...
		if(ul.css("left") == -oneLiWidth+"px"){
			ul.css({"left":-oneLiWidth*6+"px"});
			ul.stop().animate({"left":startLeft});
			i=0;
		}
		else{
	...
	rBtn.click(function(){
		...
		//边界判定
		if(ul.css("left") <= -oneLiWidth*9+"px"){
			//console.log(1);
			ul.css({"left":0+"px"});
			j = 4;
